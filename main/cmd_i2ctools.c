...
int read_i2c(uint8_t i2c_address , uint8_t comm, uint8_t data_len , uint8_t ** byte_data_ptr)
{
    ...
    *byte_data_ptr = malloc(len);
    if(*byte_data_ptr == 0)
    {
        printf("cant create byte array buffer...\n");
    }
    ...
    i2c_master_write_byte(cmd, chip_addr << 1 | READ_BIT, ACK_CHECK_EN);
    if (len > 1) {
        i2c_master_read(cmd, (*byte_data_ptr), len - 1, ACK_VAL);
    }
    i2c_master_read_byte(cmd, (*byte_data_ptr) + len - 1, NACK_VAL);
    i2c_master_stop(cmd);
    esp_err_t ret = i2c_master_cmd_begin(i2c_port, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    if (ret == ESP_OK) {
